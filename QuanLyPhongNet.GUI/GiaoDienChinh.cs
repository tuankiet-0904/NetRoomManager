...
namespace QuanLyPhongNet.GUI
{
    public partial class GiaoDienChinh : Form
    {
        ...
        {
            drgvUsingClient.DataSource = (from client in servermanager.usingClient select new { ComputerName = client.nameClient, UsingAccount = client.nameCustomer,
                State = client.stateClient, StartTime = client.startTime }).ToArray();
        }
        ...
        private void picSendMessage_Click(object sender, EventArgs e)
        {
            ...
            {
                if (drgvUsingClient.CurrentRow.Cells[2].Value.ToString().Equals("MEMBER USING"))
                {
                    string clientName = drgvUsingClient.CurrentRow.Cells[0].Value.ToString();
